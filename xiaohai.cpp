...
#include "./include/makedirectory.h"
...
int main(int argc,char **argv)
{
  ...
  string result = "/home/xiaohai/Ribll/output/";
  string resultDirectory;
  if (argc > 1) resultDirectory = result + argv[1];
  else resultDirectory = result;
  xiaohai::GenerateDirectory(resultDirectory.c_str());
  
  /// generate a chain.
  if (argc > 2) inputFile = argv[2];
  else inputFile = "../input/input.list";

  if (argc > 3) si1_outputFileP = argv[3];
  else si1_outputFileP = resultDirectory + "/si1_60_p_outputData.txt";
  if (argc > 4) si1_outputFileN = argv[4];
  else si1_outputFileN = resultDirectory + "/si1_60_n_outputData.txt";

  if (argc > 5) si2_outputFileP = argv[5];
  else si2_outputFileP = resultDirectory + "/si2_60_p_outputData.txt";
  if (argc > 6) si2_outputFileN = argv[6];
  else si2_outputFileN = resultDirectory + "/si2_60_n_outputData.txt";

  if (argc > 7) si4_outputFileP = argv[7];
  else si4_outputFileP = resultDirectory + "/si4_1000_p_outputData.txt";
  if (argc > 8) si4_outputFileN = argv[8];
  else si4_outputFileN = resultDirectory + "/si4_1000_n_outputData.txt";

  if (argc > 9) si3_outputFile = argv[9];
  else si3_outputFile = resultDirectory + "/si3_qsd_outputData.txt";
  ...
  for(unsigned int i = 0; i != CHANNEL_16; i++)
    ...
    sprintf(kedu_name_n, "kedu_si2_60_n%d", i);
      ...
      sp_1[i] = new TSpectrum(3);
      sn_1[i] = new TSpectrum(3);
      sp_2[i] = new TSpectrum(3);
      sn_2[i] = new TSpectrum(3);
      sp_4[i] = new TSpectrum(3);
      sn_4[i] = new TSpectrum(3);
      snp_3[i] = new TSpectrum(3);
    }
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++)
    {
      ...
      for (unsigned int i = 0; i != CHANNEL_16; ++i)
      	{
	  if (vmod104->chdata[i] > THRESHOLDENERGY)
	    {
	      si1_60_p[i] = vmod104->chdata[i];
	      h_si1_60_p[i]->Fill(si1_60_p[i]);
	    }
          if (vmod104->chdata[i+CHANNEL_16] > THRESHOLDENERGY)
            {
      ...
      for (unsigned int i = 0; i != CHANNEL_16; ++i)
      	{
          if (vmod106->chdata[i] > THRESHOLDENERGY)
            {
      	  ...
      	  if (i < 8)
      	    {
              if (vmod106->chdata[23-i] > THRESHOLDENERGY)
                {
      	    ...
      	    {
              if (vmod106->chdata[39-i] > THRESHOLDENERGY)
                {
      ...
      for (unsigned int i = 0; i != CHANNEL_16; ++i)
      	{
          if (vmod108->chdata[i] > THRESHOLDENERGY)
            {
              si4_1000_p[i] = vmod108->chdata[i];
              h_si4_1000_p[i]->Fill(si4_1000_p[i]);
            }
          if (vmod108->chdata[i+CHANNEL_16] > THRESHOLDENERGY)
            {
  ...
  for (unsigned i = 0; i != CHANNEL_16; ++i)
    {
      ...
      // for (int j = 0; j != 6; ++j)
  	{
  	  kedu_si2_60_p[i]->Fill(X[j], alphaEnergy[j]);
  	}
      xiaohai::LineFit *linefit = new xiaohai::LineFit();
      TF1 *fitfunc = new TF1("fitfunc", linefit, 1, 4095, 2);
  ...
  for (unsigned i = 0; i != CHANNEL_16; ++i)
    {
      ...
      // for (int j = 0; j != 6; ++j)
  	{
  	  kedu_si2_60_n[i]->Fill(X[j], alphaEnergy[j]);
  	}
      xiaohai::LineFit *linefit = new xiaohai::LineFit();
      TF1 *fitfunc = new TF1("fitfunc", linefit, 1, 4095, 2);
  ...
  for (unsigned i = 0; i != CHANNEL_16; ++i)
    {
      ...
      // for (int j = 0; j != 6; ++j)
  	{
  	  kedu_si4_1000_p[i]->Fill(X[j], alphaEnergy[j]);
  	}
      xiaohai::LineFit *linefit = new xiaohai::LineFit();
      TF1 *fitfunc = new TF1("fitfunc", linefit, 1, 4095, 2);
  ...
  for (unsigned i = 0; i != CHANNEL_16; ++i)
    {
      ...
      // for (int j = 0; j != 6; ++j)
  	{
  	  kedu_si4_1000_n[i]->Fill(X[j], alphaEnergy[j]);
  	}
      xiaohai::LineFit *linefit = new xiaohai::LineFit();
      TF1 *fitfunc = new TF1("fitfunc", linefit, 1, 4095, 2);
  ...
  //////////////////////////////////////////////////////////////////////  
  /// deal with si3_qsd300
  ofstream si3_qsd300_outputData(si4_outputFileP);
  si3_qsd300_outputData << setw(20) <<  "Si4 p number" << "   "
                        << setw(20) << "first parameter" << "   "
                        << setw(20) << "second parameter" << "   "
                        << setw(20) << "X-axis intercept"
                        << endl;
  for (unsigned i = 0; i != CHANNEL_4; ++i)
    {
      snp_3[i]->Search(h_si3_qsd300[i], 1, "same", 0.05);
      const int PEAKS = snp_3[i]->GetNPeaks();
      double *X = new double[PEAKS];
      for (int j = 0; j != PEAKS; ++j)
  	{
  	  X[j] = *(snp_3[i]->GetPositionX()+j);
  	}
      sort(X, X+PEAKS);
      for (int j = 0; j != PEAKS; ++j)
      // for (int j = 0; j != 6; ++j)
  	{
  	  kedu_si3_qsd300[i]->Fill(X[j], alphaEnergy[j]);
  	}
      xiaohai::LineFit *linefit = new xiaohai::LineFit();
      TF1 *fitfunc = new TF1("fitfunc", linefit, 1, 4095, 2);
      kedu_si3_qsd300[i]->Fit(fitfunc);
      /// Set Points.
      fitfunc->SetNpx(1000);
      si3_qsd300_outputData << setw(20) << i << "   "
                            << setw(20) << fitfunc->GetParameter(0) << "   "
                            << setw(20) << fitfunc->GetParameter(1) << "   "
                            << setw(20) << -(fitfunc->GetParameter(0))/fitfunc->GetParameter(1)
                            << endl;
  ...
  si3_qsd300_outputData.close();

  
  

  TFile *saveFile = new TFile((resultDirectory + "/result.root").c_str(), "RECREATE");
