...
int main()
{
  ...
  float scaler[CHANNEL_32];
  float si1_60_p[CHANNEL_16],si1_60_n[CHANNEL_16],si2_60_p[CHANNEL_16],
    si2_60_n[CHANNEL_16], si3_qsd300[CHANNEL_16],si4_1000_p[CHANNEL_16],
    si4_1000_n[CHANNEL_16];
  float csi_adc_1[CHANNEL_16],csi_adc_2[CHANNEL_16],csi_adc_3[CHANNEL_16];
  float tof_si_tdc[CHANNEL_16],csi_tdc_1[CHANNEL_16],csi_tdc_2[CHANNEL_16],
    csi_tdc_3[CHANNEL_32];
  ...
  TH2F *h_TOF_deltaE_p=new TH2F("h_TOF_deltaE_p","h_TOF_deltaE_p",120,80,200,1100,400,1500);
  TH2F *h_TOF_deltaE_n=new TH2F("h_TOF_deltaE_n","h_TOF_deltaE_n",120,80,200,1100,400,1500);
  TH2F *h_si1_O16_posi=new TH2F("h_si1_O16_posi","h_si1_O16_posi",16,0,16,16,0,16);
  TH2F *h_si2_O16_posi=new TH2F("h_si2_O16_posi","h_si2_O16_posi",16,0,16,16,0,16);
  TH2F *h_si3_O16_posi=new TH2F("h_si3_O16_posi","h_si3_O16_posi",2,0,2,2,0,2);
  TH2F *h_si4_O16_posi=new TH2F("h_si4_O16_posi","h_si4_O16_posi",16,0,16,16,0,16);
  ...
  for (int ii=StartNum;ii<=EndNum;ii++)
      ...
      for(int ievent =0; ievent<nentries; ievent++)//begin event loop
	{
	  ...
	  /// vmod103是用来干吗的？
	  for(int i=0;i<CHANNEL_32;i++)    //scaler
	    {
	  ...
	  for(int i=0;i<CHANNEL_32;i++)
	    {
	      csi_tdc_3[i]=vmod120->chdata[i]; 
	    }

	  //      ribll_gdr->Fill();    //Output the new tree
	  //         end get data 

	  dist=((si1_60_p_max_e-865)/100)*((si1_60_p_max_e-865)/100)
	    +((tof-136.7)/10)*((tof-136.7)/10);
	  if(dist<=1) 
	    {
	      if(si1_60_p_max_e>350&&si1_60_n_max_e>350)
		{
		  h_si1_O16_posi->Fill(si1_60_p_max_index,si1_60_n_max_index); 
		}
	      if(si2_60_p_max_e>350&&si2_60_n_max_e>350)
		{
		  h_si2_O16_posi->Fill(15-si2_60_n_max_index,si2_60_p_max_index); 
		}
	      if(si3_qsd300_max_e>350)
		{
		  h_si3_O16_posi->Fill(si3_qsd300_max_x,si3_qsd300_max_y); 
		}

	      for (int i=0;i<CHANNEL_16;i++)
		{
		  if(si4_1000_p_max_e>350)
		    {
		     h_si4_O16_posi->Fill(i,15-si4_1000_p_max_index); 
		    }
		}
	  ...
	  if(si1_60_p_max_e>400&&tof>0) 
	    {
	      h_TOF_deltaE_p->Fill(tof,si1_60_p_max_e);
	      count++;
	      //    cout<<si1_60_p_max_e<<endl;
	    }
	  if(si1_60_n_max_e>400&&tof>0) h_TOF_deltaE_n->Fill(tof,si1_60_n_max_e);
  ...
  // h_si1_O16_posi->Write();
  // h_si2_O16_posi->Write();
  // h_si3_O16_posi->Write();
  // h_si4_O16_posi->Write();
  // ribll_gdr->Write();
  rootf->Write();
  rootf->Close();
  O16_purity=O16_count*1.0/count;
  cout<<"Number of total entries:"<<totalentries<<endl;
  cout<<"Number of events:"<<count<<endl;
  cout<<"Number of O16:"<<O16_count<<endl;
  cout<<"Purity of O16:"<<O16_purity*100<<"%"<<endl;
